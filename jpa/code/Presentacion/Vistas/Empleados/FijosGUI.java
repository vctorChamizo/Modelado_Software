...
public class FijosGUI extends JPanel {
	
	
	private static final long serialVersionUID = 7577743609144090261L;
	
	private String idModule = "EmpleadoFijo";
	 ...
	 * @param lista
	 */
	public FijosGUI(List<FijoBO>lista){
	 ...
	 * @param lista
	 */
	public void updateTable(List<FijoBO>lista){
			...
			for(int j = 0;j < lista.size(); j++){
				...
				this.table.setValueAt(lista.get(j).getSueldoBase(), j, 6);
				this.table.setValueAt(lista.get(j).getImpuestos(), j, 7);
	...
	private JTable createTableClientes(int f) {
		...
		String[]nombreColumnas={"ID","NSS", "Nombre", "e-Mail", "Direccion", "Departamento", "Sueldo Base", "Impuestos", "Activo"};
