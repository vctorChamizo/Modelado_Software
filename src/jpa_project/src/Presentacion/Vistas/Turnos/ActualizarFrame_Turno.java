...
public class ActualizarFrame_Turno extends JPanel{
	

	private static final long serialVersionUID = 497812086499626421L;
	
	private Double hora_entrada, hora_salida;
	private int id;
	
	/**
	 * COnstructor
	 */
	public ActualizarFrame_Turno (TurnoBO turno){
	
		this.hora_entrada = (double) turno.getHoraEntrada();
		this.hora_salida = (double) turno.getHoraSalida();
		this.id = turno.getId();
		initGUI();
				...
				//COMENZAMOS A ANADIR LOS DIFERENTES CAMPOS CORRESPONDIENTES.
					...
					JPanel hora_entradaPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 5,0));
						...
						final JTextField hora_entradaText = new JTextField();
							...
							hora_entradaText.setText(this.hora_entrada.toString());
				...
				//------------------------------------------------------------------------------------------------------------------------------------------------
					...
					JPanel hora_salidaPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 5,0));
						...
						final JTextField hora_salidaText = new JTextField();
							...
							hora_salidaText.setText(this.hora_salida.toString());
					...
					public void actionPerformed(ActionEvent e) {
						...
						try {
							...
							if(hora_entradaText.getText().isEmpty() || hora_salidaText.getText().isEmpty()) {
								...
								Controlador.getInstancia().tratarPeticion(new Contexto("ActualizarTurno", new TurnoBO(id, entrada, salida, true)));
							}
						}
